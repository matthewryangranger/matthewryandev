      ...
      <div className="grid min-h-[70vh] grid-cols-1 md:grid-cols-2 items-center">
        ...
        <div className="col-start-1 md:row-start-1">
          <h1 className="mb-8 text-[clamp(3rem,15vmin,20rem)] font-extrabold leading-none tracking-tighter" aria-label={slice.primary.first_name + " " + slice.primary.middle_name}>
            <span className="block text-slate-600">{renderLetters(slice.primary.first_name, "first")}</span>
            <span className="block text-slate-600">{renderLetters(slice.primary.middle_name, "middle")}</span>
            <span className="-mt-[.2em] block text-slate-800">{renderLetters(slice.primary.last_name, "last")}</span>
          </h1>
